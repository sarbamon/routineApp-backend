...
router.get("/search", auth, async (req, res) => {
  try {
    ...
    const results = await Promise.all(users.map(async user => {
      ...
      return { _id: user._id, username: user.username, requestStatus, requestId };
    }));
...
router.patch("/accept/:requestId", auth, async (req, res) => {
  try {
    ...
    if (request.to._id.toString() !== req.user.id.toString()) {
...
router.patch("/reject/:requestId", auth, async (req, res) => {
  try {
    ...
    if (request.to.toString() !== req.user.id.toString()) {
      return res.status(403).json({ message: "Not allowed" });
    }
...
router.get("/friends", auth, async (req, res) => {
  try {
    ...
    const friends = accepted.map(r => {
      const isRequester = r.from._id.toString() === req.user.id.toString();
